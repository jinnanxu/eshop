	...
	<div class="Hui-article">
		<article class="cl pd-20 container" style="padding-top: 0; width: 80%; font-size: 12px;">
			<div class="mt-10">
					...
					<tbody>
						...
						<c:forEach items="${page.records }" var="r" varStatus="row">
							<tr>
								...
								<td><p class="longtxt" title="${r.exp.currLocation }" style="width: 220px;">${r.exp.currLocation }</p></td>
								<td>
									<a href="/mall/admin/expdetail?oid=${r.order.orderId }"><button class="btn btn-primary btn-sm"><i class="Hui-iconfont" style="color: #fff;">&#xe695;</i>&nbsp;物流跟踪</button></a>
								</td>
							</tr>
						</c:forEach>
					</tbody>
				...
				<c:otherwise><a href="expmgr?currentpage=1">第1页</a></c:otherwise>
				</c:choose>
				<c:if test="${pageView.currentpage>2}"><a href="expmgr?currentpage=${pageView.currentpage-1}">上一页</a></c:if>
				<c:if test="${pageView.currentpage!=1&&pageView.currentpage!=pageView.totalpage}"><b><font>第${pageView.currentpage}页</font></b></c:if>
				<c:if test="${pageView.currentpage<pageView.totalpage}"><a href="expmgr?currentpage=${pageView.currentpage+1}">下一页</a></c:if>
				<c:choose>
				<c:when test="${pageView.currentpage<pageView.totalpage}"><a href="expmgr?currentpage=${pageView.totalpage}">末页</a></c:when>
